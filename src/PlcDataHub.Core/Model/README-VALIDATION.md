...
> `ColumnName` 的 63 字节上限，要连同**列名一律 ASCII**这个前提一起读：列名由生成规则（Task 3）产出，
> 中文显示名一律先转成拼音（如 `1#窑尾温度` → `p1_yao_wei_wen_du`），**列名本身保证是纯 ASCII**，
> 因此在列名校验这一步**字节数与字符数相等**——直接按**字符数 ≤ 63** 判定即可，不必处理多字节字符。
> 至于**中文列名会踩爆 63 字节**（UTF-8 下一个中文字符占 3 字节，21 个中文就到上限），
> 那是**生成规则为什么必须把中文转成拼音**的理由（针对**直接拿中文当列名**这一已被否决的方案），
> 不是对列名的约束，也不是校验层的负担。
